...
<template>
  <main v-if="user">
      ...
      <table class="w-full">
        ...
        <tbody class="">
          <tr v-for="(vocab, index) in vocabs" :key="vocab.id">
            ...
            <td>{{ vocab.user_id }}</td>
            <td>
              <button class="p-2 rounded-md bg-green-500 text-white m-1" @click="showEdit">Edit</button>
              <button class="p-2 rounded-md bg-red-500 text-white m1-1">Delete</button>
            </td>
          </tr>
        </tbody>
      </table>
    ...
    <div v-if="isShowEdit" class="fixed bg-slate-950">
      ...
      <div>
        <button class="p-2 rounded-md bg-green-600 hover:bg-green-800 text-white">Save</button>
        <button class="p-2 rounded-md bg-gray-300 hover:bg-slate-400">Cancel</button>
      </div>
    </div>
  </main>
</template>
